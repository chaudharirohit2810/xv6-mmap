...
void file_invalid_fd_test();     // Invalid fds to file backed mapping
void file_invalid_flags_test();  // Invalid flags to file backend mapping
// writeable shared mapping on read only file
void file_writeable_shared_mapping_on_ro_file_test();
// read only shared mapping on file opened in read only mode
...
void file_invalid_addr_test();  // File backed Mapping with invalid user provided address
// File backed Mapping with user provided address overlapping with existing mapping test
void file_overlap_given_addr_test();
// File backed Mapping with user provided address can be fit between two existing mappings
void file_intermediate_given_addr_test();
// File Mapping with user provided address that cannot fit between two existing mappings
...
// Private & Shared anonymous mapping together with fork test Invalid flags to mmap test
void anon_private_shared_fork_test();
void anon_given_addr_test();    // Anonymous Mapping with valid user provided address test
void anon_invalid_addr_test();  // Anonymous Mapping with invalid user provided address
// Anonymous Mapping with user provided address overlapping with existing mapping test
void anon_overlap_given_addr_test();
// Anonymous Mapping with user provided address can be fit between two existing mappings
void anon_intermediate_given_addr_test();
// Anonymous Mapping with provided address that cannot fit between two existing mappings
...
void file_invalid_fd_test() {
  ...
  for (int i = 0; i < 3; i++) {
    char *ret =
        (char *)mmap((void *)0, size, PROT_READ | PROT_WRITE, MAP_PRIVATE, fds[i], 0);
    if (ret == (void *)-1) {
      continue;
    }
...
void file_writeable_shared_mapping_on_ro_file_test() {
  ...
  if (fd == -1) {
    printf(
        1,
        "file backed writeable shared mapping on read only file test failed: at open\n");
    exit();
  }
...
void file_ro_shared_mapping_on_ro_file_test() {
  ...
  if (fd == -1) {
    printf(
        1,
        "file backed read only shared mapping on read only file test failed: at open\n");
    exit();
  }
...
void file_exceed_count_test() {
  ...
  for (; i < count; i++) {
    void *ret =
        mmap((void *)0, size, PROT_READ | PROT_WRITE, MAP_ANONYMOUS | MAP_PRIVATE, fd, 0);
    arr[i] = (uint)ret;
    if (ret == (void *)-1) {
      break;
    }
...
void file_private_test() {
  ...
  char buf2[1000];
  // Read from the file again and check if it is not equal to mapping data as mapping is
  // private
...
void file_pagecache_coherency_test() {
  ...
  char *ret2 =
      (char *)mmap((void *)0, size, PROT_READ | PROT_WRITE, MAP_PRIVATE, fd2, 100);
  if (ret2 == (void *)-1) {
...
void file_given_addr_test() {
  ...
  char *ret =
      (char *)mmap((void *)0x60001000, 200, PROT_READ | PROT_WRITE, MAP_PRIVATE, fd, 0);
  if (ret == (void *)-1) {
...
void file_invalid_addr_test(void) {
  ...
  char *ret =
      (char *)mmap((void *)0x50001000, 200, PROT_READ | PROT_WRITE, MAP_PRIVATE, fd, 0);
  if (ret != (void *)-1) {
    printf(1, "file backed invalid provided address test failed\n");
    munmap(ret, 200);
    exit();
...
void file_overlap_given_addr_test() {
  ...
  char *ret =
      (char *)mmap((void *)0x60001000, 10000, PROT_READ | PROT_WRITE, MAP_PRIVATE, fd, 0);
  if (ret == (void *)-1) {
...
void file_intermediate_given_addr_test() {
  ...
  if (ret == (void *)-1) {
    printf(
        1,
        "file backed intermediate provided address test failed: failed at first mmap\n");
    exit();
  }
  char *ret2 =
      (char *)mmap((void *)0x60003000, 200, PROT_READ | PROT_WRITE, MAP_PRIVATE, fd, 0);
  if (ret2 == (void *)-1) {
    printf(
        1,
        "file backed intermediate provided address test failed: failed at second mmap\n");
    munmap(ret, 1000);
    exit();
  }
  char *ret3 =
      (char *)mmap((void *)0x60000100, 1000, PROT_READ | PROT_WRITE, MAP_PRIVATE, fd, 0);
  if (ret3 != (void *)0x60001000) {
    printf(
        1,
        "file backed intermediate provided address test failed: failed at third mmap\n");
    munmap(ret, 1000);
    munmap(ret2, 200);
    exit();
  }
...
void file_intermediate_given_addr_not_possible_test() {
  ...
  if (ret == (void *)-1) {
    printf(1,
           "file backed intermediate provided address not possible test failed: failed "
           "at first mmap\n");
    exit();
  }
  char *ret2 =
  ...
  char *ret3 =
      (char *)mmap((void *)0x60000100, 10000, PROT_READ | PROT_WRITE, MAP_PRIVATE, fd, 0);
...
void anon_exceed_count_test(void) {
  ...
  for (; i < count; i++) {
    void *ret =
        mmap((void *)0, size, PROT_READ | PROT_WRITE, MAP_ANONYMOUS | MAP_PRIVATE, -1, 0);
    arr[i] = (uint)ret;
    if (ret == (void *)-1) {
      break;
    }
...
// Simple private anonymous mapping test with maping having both read and write permission
// and size greater than two pages
void anon_private_test() {
  printf(1, "anonymous private mapping test\n");
  int size = 10000;
  int *ret = (int *)mmap((void *)0, size, PROT_READ | PROT_WRITE,
                         MAP_ANONYMOUS | MAP_PRIVATE, -1, 0);
  if (ret == (void *)-1) {
...
void anon_shared_test() {
  ...
  int size = 10000;
  int *ret = mmap((void *)0, size, PROT_READ | PROT_WRITE, MAP_SHARED | MAP_ANONYMOUS, -1,
                  0);  // Shared mapping
  if (ret == (void *)-1) {
...
void anon_shared_multi_fork_test() {
  ...
  int size = 1000;
  char *ret = mmap((void *)0, size, PROT_READ | PROT_WRITE, MAP_SHARED | MAP_ANONYMOUS,
                   -1, 0);  // Shared mapping
  if (ret == (void *)-1) {
...
void anon_private_fork_test() {
  ...
  int size = 200;
  char *ret = mmap((void *)0, size, PROT_READ | PROT_WRITE, MAP_PRIVATE | MAP_ANONYMOUS,
                   -1, 0);  // Shared mapping
  if (ret == (void *)-1) {
...
void anon_private_shared_fork_test() {
  ...
  char *ret = mmap((void *)0, 200, PROT_READ | PROT_WRITE, MAP_ANONYMOUS | MAP_PRIVATE,
                   -1, 0);  // Private mapping
  if (ret == (void *)-1) {
    printf(1, "anonymous private & shared mapping together with fork test failed\n");
    exit();
  }
  char *ret2 = mmap((void *)0, 200, PROT_READ | PROT_WRITE, MAP_SHARED | MAP_ANONYMOUS,
                    -1, 0);  // Shared mapping
  if (ret2 == (void *)-1) {
    printf(1, "anonymous private & shared mapping together with fork test failed\n");
    exit();
...
void anon_given_addr_test() {
  ...
  char *ret = (char *)mmap((void *)0x60001000, 200, PROT_READ | PROT_WRITE,
...
void anon_invalid_addr_test(void) {
  ...
  char *ret = (char *)mmap((void *)0x50001000, 200, PROT_READ | PROT_WRITE,
                           MAP_PRIVATE | MAP_ANONYMOUS, -1, 0);
  if (ret != (void *)-1) {
...
void anon_overlap_given_addr_test() {
  ...
  char *ret = (char *)mmap((void *)0x60001000, 10000, PROT_READ | PROT_WRITE,
                           MAP_PRIVATE | MAP_ANONYMOUS, -1, 0);
  if (ret == (void *)-1) {
    printf(1, "anonymous overlapping provided address test failed: at first mmap\n");
    exit();
  }
  char *ret2 = (char *)mmap((void *)0x60001000, 200, PROT_READ | PROT_WRITE,
                            MAP_PRIVATE | MAP_ANONYMOUS, -1, 0);
  if (ret2 == (void *)-1 || ret2 == (void *)0x60001000) {
...
void anon_intermediate_given_addr_test() {
  ...
  if (ret == (void *)-1) {
    printf(1,
           "anonymous intermediate provided address test failed: failed at first mmap\n");
    exit();
  }
  char *ret2 = (char *)mmap((void *)0x60003000, 200, PROT_READ | PROT_WRITE,
                            MAP_PRIVATE | MAP_ANONYMOUS, -1, 0);
  if (ret2 == (void *)-1) {
    printf(
        1,
        "anonymous intermediate provided address test failed: failed at second mmap\n");
    munmap(ret, 1000);
    exit();
  }
  char *ret3 = (char *)mmap((void *)0x60000100, 1000, PROT_READ | PROT_WRITE,
                            MAP_PRIVATE | MAP_ANONYMOUS, -1, 0);
  if (ret3 != (void *)0x60001000) {
    printf(1,
           "anonymous intermediate provided address test failed: failed at third mmap\n");
    munmap(ret, 1000);
    munmap(ret2, 200);
    exit();
  }
...
void anon_intermediate_given_addr_not_possible_test() {
  ...
  if (ret == (void *)-1) {
    printf(1,
           "anonymous intermediate provided address not possible test failed: failed at "
           "first mmap\n");
    exit();
  }
  char *ret2 = (char *)mmap((void *)0x60003000, 200, PROT_READ | PROT_WRITE,
  ...
  char *ret3 = (char *)mmap((void *)0x60000100, 10000, PROT_READ | PROT_WRITE,
                            MAP_PRIVATE | MAP_ANONYMOUS, -1, 0);
  if (ret3 == (void *)0x60000100) {
    printf(1,
           "anonymous intermediate provided address not possible test failed: failed at "
...
void mmap_write_on_ro_mapping_test() {
  ...
  if (pid == 0) {
    int size = 10000;
    int *ret =
        (int *)mmap((void *)0, size, PROT_READ, MAP_ANONYMOUS | MAP_PRIVATE, -1, 0);
    if (ret == (void *)-1) {
...
void mmap_none_permission_test() {
  ...
  if (pid == 0) {
    int size = 10000;
    char *ret = (char *)mmap((void *)0x70003000, size, PROT_NONE,
                             MAP_ANONYMOUS | MAP_PRIVATE, -1, 0);
    if (ret == (void *)-1) {
      printf(1, "none permission on mapping test failed\n");
      exit();
...
void mmap_valid_map_fixed_test() {
  ...
  char *ret = mmap((void *)0x60001000, 200, PROT_WRITE | PROT_READ,
...
void mmap_invalid_map_fixed_test() {
  ...
  char *ret = mmap((void *)0x50001000, 200, PROT_WRITE | PROT_READ,
                   MAP_FIXED | MAP_PRIVATE | MAP_ANONYMOUS, -1, 0);
  if (ret != (void *)-1) {
  ...
  char *ret2 = mmap((void *)0x60000100, 200, PROT_WRITE | PROT_READ,
                    MAP_FIXED | MAP_PRIVATE | MAP_ANONYMOUS, -1, 0);
  if (ret2 != (void *)-1) {
    printf(1, "mmap invalid address map fixed flag test failed\n");
    exit();
  ...
  char *ret3 = mmap((void *)0x60000000, 200, PROT_WRITE | PROT_READ,
                    MAP_FIXED | MAP_PRIVATE | MAP_ANONYMOUS, -1, 0);
  if (ret3 == (void *)-1) {
    printf(1, "mmap invalid address map fixed flag test failed\n");
    exit();
  }
  char *ret4 = mmap((void *)0x60000000, 200, PROT_WRITE | PROT_READ,
                    MAP_FIXED | MAP_PRIVATE | MAP_ANONYMOUS, -1, 0);
  if (ret4 != (void *)-1) {
    printf(1, "mmap invalid address map fixed flag test failed\n");
    exit();
